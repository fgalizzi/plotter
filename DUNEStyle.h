...
namespace dunestyle
{
  ...
  namespace colors
  {
    ...
    inline Color_t NextColor(Cycle cycle = Cycle::OkabeIto, int start=-1)
    {
  ...
  /// \param inFrame  When using "top" vertical alignment, specifies whether the label is inside the plot frame or outside it
  /// \return      The TLatex instance for the text
  inline TLatex* WIP(ETextAlign loc=static_cast<ETextAlign>(kHAlignLeft + kVAlignTop), bool inFrame=true)
  {
  ...
  /// \param inFrame  When using "top" vertical alignment, specifies whether the label is inside the plot frame or outside it
  /// \return      The TLatex instance for the text
  inline TLatex* Simulation(ETextAlign loc=static_cast<ETextAlign>(kHAlignLeft + kVAlignTop), bool inFrame=true)
  {
  ...
  /// \param inFrame  When using "top" vertical alignment, specifies whether the label is inside the plot frame or outside it
  /// \return      The TLatex instance for the text
  inline TLatex* Official(ETextAlign loc=static_cast<ETextAlign>(kHAlignLeft + kVAlignTop), bool inFrame=true)
  {
  ...
  inline void CherryInvertedPalette()
  {
  ...
  inline void BlueWhiteRedPalette()
  {
    const int NRGBs = 3;
    const int n_color_contours = 50;
  ...
  inline void SplitCanvas(TCanvas * c, double ysplit, TPad*& p1, TPad*& p2)
  {
    c->cd();
    if (!p1)
      p1 = new TPad("", "", 0, 0, 1, 1);
  ...
  inline std::vector<TGraph*> GetContourGraphs(const TH2* h2, const double level)
  {
  ...
  inline bool SetDuneStyle()
  {
    ...
    // Axis titles
    duneStyle->SetTitleSize(kDuneSize, "xyz");
    duneStyle->SetTitleOffset(1.5, "xyz");
    duneStyle->SetTitleOffset(0.7, "z");

    // This applies the same settings to the overall plot title
    duneStyle->SetTitleSize(kDuneSize, "");
    duneStyle->SetTitleOffset(1.5, "");

    // Axis labels (numbering)
    duneStyle->SetLabelSize(kDuneSize, "xyz");
  ...
  inline void ApplyDuneStyle(TH1* h) {
    const int kDuneFont = 43;
    const float kDuneSize = 24;
    h->SetLabelFont(kDuneFont, "xyz");
    h->SetTitleFont(kDuneFont, "xyz");
    h->SetTitleFont(kDuneFont, ""); // Apply same setting to plot titles

    // Axis titles
    h->SetTitleSize(kDuneSize, "xyz");
    h->SetTitleOffset(1.5, "xy");
    h->SetTitleOffset(0.7, "z");

    // This applies the same settings to the overall plot title
    h->SetTitleSize(kDuneSize, "");
    h->SetTitleOffset(1.5, "");

    // Axis labels (numbering)
    h->SetLabelSize(kDuneSize, "xyz");
    h->SetLabelOffset(.005, "xyz");
